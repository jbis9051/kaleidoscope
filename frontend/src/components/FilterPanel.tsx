...
export default function FilterPanel({api, filter, trashEnabled, setFilter, onTrash, onSave}: FilterPanelProps) {
    ...
    const [fullSearchInput, setFullSearchInput] = useState<string>("");
    ...
    useEffect(() => {
        ...
        const fullSearch = filter.get("full_search", "%");
        if (fullSearch !== null && fullSearch.startsWith("%") && fullSearch.endsWith("%")) {
            setFullSearchInput(fullSearch.substring(1, fullSearch.length - 1));
        }
    }, [filter])
    ...
    function fullSearchGo(){
        if (!filterError) {
            const currentFilter = Filter.fromString(filterInput);
            if (fullSearchInput.length > 0) {
                currentFilter.set("full_search", "%", `%${fullSearchInput}%`);
            } else {
                currentFilter.set("full_search", "%", null);
            }
            ...
            <div className={styles.filterPanel}>
                <div className={styles.filter}>
                    <input
                        ...
                        onChange={(e) => {
                            setFullSearchInput(e.target.value);
                        }}
                        onKeyDown={(e) => {
                            if (e.key === "Enter") {
                                e.preventDefault();
                ...
                <div className={styles.actions}>
                    <div>
                        <button onClick={() => fullSearchGo()}>Search
                        </button>
                    </div>
                </div>
            </div>
                        ...
                        onKeyDown={(e) => {
                            ...
                            if (e.key === "Enter") {
                                ...
                                if (selectedSuggestion >= 0 && selectedSuggestion < suggestions.length) {
                                    ...
                                    setFilter(Filter.fromString(filterInput))
                                }
                            }
                        }}
                        onBlur={() => {
                    ...
                    <div className={styles.suggestions}>
                        {suggestions.map((s, i) =>
                            <div
                                ...
                                key={s}
                                className={`${styles.suggestion} ${i === selectedSuggestion && styles.selected}`}>{s}</div>)}
                    </div>
                }
